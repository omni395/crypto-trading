...
- **Пользовательские фильтры и настройки** через модальное окно (котируемая валюта, статус, биржа и др.)
...
## Важно: обновление фильтров и удаление market_type

- Поле `market_type` полностью удалено из базы, моделей, фильтров и интерфейса.
- Фильтрация монет теперь работает только по бирже (`exchange`), статусу и котируемой валюте.
- Все упоминания типа рынка убраны из модалки, фильтров, настроек пользователя, компонентов и JS.
- При добавлении новой биржи её тип рынка хранится только в Exchange и не влияет на фильтрацию монет.

## Обновления по формам и админке

- Формы создания и редактирования бирж приведены к единому современному стилю с формами ролей: теперь используются одинаковые контейнеры, стили полей, кнопки и структура.
- Заголовки, кнопки и поля теперь визуально едины во всех административных разделах.
- Ошибки валидации и возврат к списку реализованы аналогично другим формам админки.
...
- **Выбор биржи теперь полностью динамический:** поле "Биржа по умолчанию" отображает актуальный список из базы, фильтрация производится по выбранной бирже.
- Все остальные фильтры (валюта, статус, объем, сделки, изменение, цена) работают в обычном режиме.
...
2. Открывает модальное окно настроек, меняет фильтры (валюта, статус и пр.), сохраняет — настройки отправляются через API, список монет обновляется.
...
- Настройки пользователя (exchange, quote_asset, status) сохраняются через модальное окно.
...
- Используется Redis для кеширования часто используемых данных:
  ...
  - Уникальные значения для фильтров (exchange, status, quote_asset) — TTL 10 минут
...
- **2025.04:**
  ...
  - Кеширование справочников фильтров (exchange, status, quote_asset)
